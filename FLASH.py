...
TELEGRAM_BOT_TOKEN = '7703225179:AAG4BoY99pERBLLwfh_VDWg8jktZhwzOyUc'
ADMIN_USER_ID = 5786411100  
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ You are not authorized to use this bot! /get_id I'd le or baap ko bej @TG_FLASH92*", parse_mode='Markdown')
        return
    message = (
...
async def add_user(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="*❌ You are not authorized to add users @TG_FLASH92!*", parse_mode='Markdown')
        return
...
async def remove_user(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="*❌ You are not authorized to remove users @TG_FLASH92!*", parse_mode='Markdown')
        return
